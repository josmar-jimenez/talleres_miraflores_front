    ...
    this.serviceUse.update(this.id, product_data).subscribe(
      (response: any) => {
        ...
        if (existeError) {
          ...
          sms = this.translate.instant('product').concat(" ").concat(prop_glo.sms_component.sms_success_edit);
          pref = prop_glo.sms_component.pref_exito;
          if (this.imageChanged) {
            this.storageService.uploadBytes("product/" + this.id + "/image.jpg", this.fileToUpload);
          }
        }
    ...
    this.serviceUse.delete(this.id).subscribe(
      (response: any) => {
        ...
        if (existeError) {
          ...
          sms = this.translate.instant('product').concat(" ").concat(prop_glo.sms_component.sms_success_delete);
          pref = prop_glo.sms_component.pref_exito;
          this.storageService.delete("product/" + this.id + "/image.jpg");
        }
